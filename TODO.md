...
- [x] **VIO-M3-006** Implement `async_semaphore`.
- [x] **VIO-M3-007** Implement `async_mutex` without resuming waiters while holding its internal lock.
...
- [x] **VIO-M4-010** Run the same backend contract suite against the virtual backend and epoll.

## M5 — Files and Blocking Work

- [x] **VIO-M5-001** Implement a bounded blocking executor whose full queue returns a resource error.
- [x] **VIO-M5-002** Implement asynchronous file open, close, `read_at`, and `write_at`.
...
- [x] **VIO-M6-008** Define default-enable criteria with benchmarks and fall back when capabilities are insufficient.
...
- [x] **VIO-M8-002** Add TSan jobs covering tasks, scopes, channels, mailboxes, and backends.
